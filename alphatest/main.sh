...
#=======Если нода синхронизирована - начинаем работу ==========
  ...
  do 	
	  ...
	  then		
	  	  ...
	  	  (echo ${PASSWALLET}) | $binary tx staking create-validator --amount="800000$denom" --pubkey=$($binary tendermint show-validator) --moniker="$MONIKER"	--chain-id="$chain" --commission-rate="0.10" --commission-max-rate="0.20" --commission-max-change-rate="0.01" --min-self-delegation="800000" --gas="auto"	--from="$address" --fees="5550$denom" -y
	  	  sleep 20
	  	  val=`$binary query staking validator $valoper -o json | jq -r .description.moniker`
	  	  echo $val
	  	  sed -i.bak -e "s/val=/val=$val/;" $HOME/.bashrc		
